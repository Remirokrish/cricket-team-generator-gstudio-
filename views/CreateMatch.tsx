...
const CreateMatch: React.FC<CreateMatchProps> = ({
  players,
  onSaveState,
  initialState,
}) => {
  ...
  const [selectedIds, setSelectedIds] = useState<Set<string>>(
    new Set(initialState.selectedPlayerIds),
  );
  const [captainA, setCaptainA] = useState<string | null>(
    initialState.captainAId,
  );
  const [captainB, setCaptainB] = useState<string | null>(
    initialState.captainBId,
  );
  const [teams, setTeams] = useState<GeneratedTeams | null>(
    initialState.lastGeneratedTeams,
  );
  ...
  const generateTeams = () => {
    ...
    const pool = selectedPlayers.filter(
      (p) => p.id !== captainA && p.id !== captainB,
    );
  ...
  if (step === "select-players") {
    return (
      <div className="flex flex-col h-full bg-slate-950 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 min-h-0 no-scrollbar">
          {players.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {players.map((player) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-5 h-5 rounded border-2 flex items-center justify-center mr-3 transition-colors ${isSelected ? "bg-blue-500 border-blue-500" : "border-slate-600 bg-slate-800"}`}
                      >
                        {isSelected && <Check className="w-3 h-3 text-white" />}
                      </div>
                      <span
                        className={`font-medium tracking-wide ${isSelected ? "text-blue-200" : "text-slate-300 group-hover:text-white"}`}
                      >
                        {player.fullName}
                      </span>
                      {isSelected && (
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="bg-slate-900 p-8 rounded-2xl shadow-xl border border-blue-500/30 space-y-8 relative overflow-hidden mb-10">
              ...
              <div className="flex items-center justify-center">
                ...
                <span className="px-4 text-slate-500 font-bold text-xs uppercase">
                  VS
                </span>
                <div className="h-px bg-slate-700 w-full"></div>
              </div>
              ...
              <div className="relative">
                <label className="block text-sm font-bold text-cyan-300 mb-2 flex items-center gap-2 uppercase tracking-wide">
                  <Crown className="w-5 h-5 text-cyan-500 fill-cyan-500 drop-shadow-md" />{" "}
                  Captain Team B
                </label>
                <select
                  value={captainB || ""}
    ...
    return (
      <div className="flex flex-col h-full bg-slate-950 overflow-hidden">
        <div className="flex-none p-4 md:p-6 bg-slate-900 border-b border-blue-500/20 sticky top-0 z-20 flex justify-between items-center shadow-lg">
          <button
            onClick={() => setStep("select-captains")}
            className="text-slate-400 hover:text-white text-sm flex items-center font-medium transition-colors"
          >
            <ChevronLeft className="w-4 h-4 mr-1" /> Edit
          </button>
          <h2 className="font-bold text-white tracking-widest text-lg drop-shadow-[0_0_5px_rgba(255,255,255,0.3)]">
            MATCH TEAMS
          </h2>
          <button
            onClick={() => setShowToss(true)}
            className="text-sm font-bold text-yellow-400 bg-yellow-900/30 border border-yellow-500/30 px-3 py-1.5 rounded-full hover:bg-yellow-900/50 hover:shadow-[0_0_10px_rgba(234,179,8,0.4)] transition-all flex items-center"
          >
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 min-h-0">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-slate-900 rounded-2xl shadow-neon-blue border border-blue-500/50 overflow-hidden flex flex-col h-full relative">
              ...
              <div className="bg-blue-900/40 p-5 backdrop-blur-sm border-b border-blue-500/30 flex justify-between items-center">
                <h3 className="font-black text-xl text-blue-100 tracking-wider italic uppercase">
                  TEAM A
                </h3>
                <span className="text-xs bg-blue-600/80 text-white px-3 py-1 rounded-full font-bold shadow-md">
              ...
              <ul className="divide-y divide-slate-800 p-2 flex-1 bg-slate-900/50">
                {teams.teamA.map((p) => (
                  <li
                    key={p.id}
                    className="p-3 flex items-center gap-3 hover:bg-white/5 rounded-lg transition-colors"
                  >
                    {p.id === teams.captainA?.id && (
                      <Crown className="w-4 h-4 text-yellow-400 fill-yellow-400 flex-shrink-0 drop-shadow-md" />
                    )}
                    {p.id === teams.commonPlayer?.id && (
                      <span className="bg-purple-900 text-purple-300 border border-purple-500 text-[10px] uppercase font-bold px-1.5 rounded flex-shrink-0">
                        Common
                      </span>
                    )}
                    <span
                      className={`${p.id === teams.captainA?.id ? "font-bold text-yellow-100" : "text-slate-300"} tracking-wide`}
                    >
                      {p.fullName}
                    </span>
                  </li>
            ...
            <div className="bg-slate-900 rounded-2xl shadow-neon-cyan border border-cyan-500/50 overflow-hidden flex flex-col h-full relative">
              ...
              <div className="bg-cyan-900/40 p-5 backdrop-blur-sm border-b border-cyan-500/30 flex justify-between items-center">
                <h3 className="font-black text-xl text-cyan-100 tracking-wider italic uppercase">
                  TEAM B
                </h3>
                <span className="text-xs bg-cyan-600/80 text-white px-3 py-1 rounded-full font-bold shadow-md">
              ...
              <ul className="divide-y divide-slate-800 p-2 flex-1 bg-slate-900/50">
                {teams.teamB.map((p) => (
                  <li
                    ...
                    className="p-3 flex items-center gap-3 hover:bg-white/5 rounded-lg transition-colors"
                  >
                    {p.id === teams.captainB?.id && (
                      <Crown className="w-4 h-4 text-cyan-400 fill-cyan-400 flex-shrink-0 drop-shadow-md" />
                    )}
                    {p.id === teams.commonPlayer?.id && (
                      <span className="bg-purple-900 text-purple-300 border border-purple-500 text-[10px] uppercase font-bold px-1.5 rounded flex-shrink-0">
                        Common
                    ...
                    <span
                      className={`${p.id === teams.captainB?.id ? "font-bold text-cyan-100" : "text-slate-300"} tracking-wide`}
                    >
                      {p.fullName}
                    </span>
                  </li>
          ...
          <div className="mt-8 pt-6 border-t border-slate-800">
            <Button
              ...
              fullWidth
              onClick={generateTeams}
              className="border-dashed border-slate-600 text-slate-400 hover:text-white hover:border-white"
            >
